...
namespace DAL.Factory.Web.ExportEquipment
{
    public class ExportEquipmentManagement : BaseManagement
    {
        public ExportEquipmentManagement()
        {
            db = new QRMSEntities();
        }

        public ExportEquipmentManagement(QRMSEntities db)
        {
            this.db = db ?? DataContext.getEntities();
        }

        public HDLIB.WebPaging.TPaging<DAL.TransactionHistory> FindAllExportEquipment(int page, int limit,
            string itemCode, string orderNo, string wareHouseCode, DateTime? startDate, DateTime? endDate)
        {
            try
            {
                ...
                query = query.Where(n => n.RecordStatus != null && n.RecordStatus != ConstRecordStatus.Deleted && n.TransactionType == ConstTransactionType.Export);
