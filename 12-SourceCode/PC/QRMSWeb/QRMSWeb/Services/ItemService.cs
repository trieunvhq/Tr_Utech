...
{
    public class ItemService : APIService
    {
        public ItemService(HttpClient client) : base(client)
        {
        }
        ...
        {
            string url = $"api-wa/item/all-items";
            var response = await Client.GetAsync(url);
            this.checkResponse(response);
            string responseBody = await response.Content.ReadAsStringAsync();
            var responseData = JsonConvert.DeserializeObject<ResponseData<List<ItemModel>>>(responseBody);
