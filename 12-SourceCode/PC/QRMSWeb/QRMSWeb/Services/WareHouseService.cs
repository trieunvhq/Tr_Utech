...
{
    public class WareHouseService : APIService
    {
        ...
        {
            string url = $"api-wa/ware-house/all-warehouses";
            var response = await Client.GetAsync(url);
            this.checkResponse(response);
            string responseBody = await response.Content.ReadAsStringAsync();
            var responseData = JsonConvert.DeserializeObject<ResponseData<List<WareHouseModel>>>(responseBody);
