...
{
    public class ExportEquipmentService : APIService
    {
        public ExportEquipmentService(HttpClient client) : base(client)
        {
        }
        public async Task<PaginateData<List<TransactionHistoryModel>>> SearchExportEquipment(int page, int rowPerPage,
            string itemCode, string OrderNo, string wareHouseCode, string startDate, string endDate)
        {
            ...
            var response = await Client.GetAsync("api-wa/export-equipment/find-all-export-equipment?" + queryString);
